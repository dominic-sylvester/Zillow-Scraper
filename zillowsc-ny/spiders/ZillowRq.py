...
class ZillowSpiderMan(scrapy.Spider):
    ...
    def start_requests(self):
        with open('Z2SpiderMan.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["Zip Code", "Price", "Status", "URL"])
    ...
    def parse(self, response):
        ...
        lists = card_li.css('a.list-card-link::attr(href)').extract()
    ...
    def formatter(self, response):
        ...
        with open('Z2SpiderMan.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(
                [zip_code, price, status, urlnow])
